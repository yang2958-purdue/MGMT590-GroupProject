...
- **Content script bundle** — The content script is built as an ES module with a shared chunk (`chunks/fieldInference-*.js`). After programmatic inject, the side panel waits on animation frames and retries `sendMessage` until the module finishes loading (so `onMessage` is registered). The DOM scanner recurses into **same-origin iframes** and through **open shadow roots** (Workday often nests inputs under web components), and passes `iframePath` so fills target the correct document. A **second pass** walks the same trees and picks up Work Experience **date** controls the generic query omits (**`disabled`** inputs, **`input type=button`**, and matches by **`workExperience` + row index + date/month/year** on id / `data-automation-id` / `aria-label`, not only fixed CSS `[id*=…]` substrings). Field-to-resume mapping **concatenates** label, **`id`** (Workday encodes **`startDate`** / **`endDate`** there), placeholder, `data-automation-id`, and name so scoped inference succeeds when the visible label is only e.g. “Month”.
...
- **Skills (heuristic)** — Beyond a dedicated Skills section, the client parser pulls skill-like tokens from **work experience** and **projects** blocks (comma lists, parentheticals, lines like `Technologies: …`). LLM parsing (`/parse-resume-llm`) is instructed to include tools mentioned in experience and projects, not only a Skills heading; heuristic + LLM skills are merged with deduplication.
- **Work experience autofill** — The mapper fills **Job Title**, **Company**, **Location** (from each job’s parsed `experience[].location`), **Role Description** (first job’s bullets), **From** / **To** (split `dates` into MM/YYYY; supports en-dashes and `Aug 2020 - Present`), and **I currently work here** when `dates` includes Present/Current. Repeater rows use Workday ids like `workExperience-0--` / `workExperience-11--`; scoped field inference matches **any** index (`workExperience\d+`) and still maps values from **resume `experience[0]`** into the active row. The DOM scanner classifies **textarea** and **contenteditable** before `role="combobox"` so Role Description is not treated as a dropdown. The filler resolves an inner **`<textarea>`** when the mapped node is still typed as a dropdown but contains a textarea, and toggles **I currently work here** via the real `input[type=checkbox]` / label click (not only `.checked`). Workday date inputs: split month/year (`datesection*`), broader Workday `workExperience` id hints, and combined `MM/YYYY` text fields when scoped to work experience. **Month** fields implemented as Workday listboxes get search tokens derived from the parsed segment (month names and `MM`, not only `MM/YYYY`), and **year** listboxes get the **year** digit so they are not confused with month options. Optional override: **Settings** resume overrides `workLocation`.
...
- **Autofill pipeline** — Open the **real** application site in a normal browser tab (e.g. Workday, Greenhouse). In the side panel, go to **Autofill** and press **Autofill this tab** to extract fields and fill using pause/resume/skip controls. The job **Detail** page only offers **Open job posting** (listing URL); it does not start autofill. Field discovery uses **Firecrawl** when the site is allowed and `VITE_FIRECRAWL_API_KEY` is set in `.env.local` (see `.env.example`); otherwise the extension scans the **live tab DOM**. **LinkedIn** is skipped for remote Firecrawl extract — rely on DOM scan on the application page. On **Workday-style** forms, if your parsed resume has work or education entries, the extension searches the live DOM (including **shadow roots** and **nested same-origin iframes**) for **Add** controls tied to those sections, clicks them, waits for the DOM to settle, and **rescans** so repeater row fields exist before mapping. Skills are filled from the resume `skills` list (comma-joined) into Workday-style “add skills” inputs when detected, with up to 24 tags per run. Mapping uses label + control type (e.g. **select** options matched by text/value; phone/email are not applied to yes/no controls). Edit **Settings** to maintain `jobbot_userProfile` (answers, resume overrides, custom keys). Rebuild after changing env vars (`npm run build`).
